...
(function () {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const API_URL = '/api'; // ALTERAÇÃO CRÍTICA
        ...
        const viewRenderer = {
            async dashboard(container) {
                if (appState.secoes.length === 0) {
                    try { appState.secoes = await apiService.get('/admin/secoes'); } 
                ...
                container.innerHTML = `
                    ...
                    <div class="dashboard-grid dashboard-grid-3-cols">
                        ...
                        <div class="card kpi-card"><h3>Total Empenhado (Líquido)</h3><p id="kpi-valor-empenhado">A carregar...</p></div>
                    </div>
                    <div class="card aviso-card"><h3><i class="fas fa-exclamation-triangle"></i> Avisos: NCs com Prazo Próximo (7 dias)</h3><div id="aviso-content">A carregar...</div></div>
                    <div class="card">
                        <div class="view-header">
                            <h3>Gerar Relatório Geral em PDF</h3>
                        </div>
                        <div class="filters">
                            <div class="filter-group"><label for="report-filter-pi">Plano Interno</label><input type="text" id="report-filter-pi" placeholder="Ex: 2024NE123456"></div>
                            <div class="filter-group"><label for="report-filter-nd">Natureza de Despesa</label><input type="text" id="report-filter-nd" placeholder="Ex: 339030"></div>
                            ...
                            <div class="filter-group"><label for="report-filter-status">Status</label><select id="report-filter-status"><option value="">Todos</option><option value="Ativa">Ativa</option><option value="Totalmente Empenhada">Totalmente Empenhada</option><option value="Recolhida">Recolhida</option></select></div>
            ...
            async notasCredito(container, page = 1) {
                    ...
                    <div class="card">
                        <div class="filters">
                            <div class="search-bar">
                                ...
                                <input type="search" id="search-nc" placeholder="Buscar por Nº da NC, PI ou ND...">
                            </div>
                            ...
                            <div class="filter-group"><label for="filter-status">Status</label><select id="filter-status"><option value="">Todos</option><option value="Ativa">Ativa</option><option value="Totalmente Empenhada">Totalmente Empenhada</option><option value="Recolhida">Recolhida</option></select></div>
                ...
                container.querySelector('#apply-filters-btn').addEventListener('click', () => {
                    ...
                    appState.searchTerms.nc = container.querySelector('#search-nc').value;
                ...
                container.querySelector('#search-nc').addEventListener('keydown', (e) => {
                    if (e.key === 'Enter') {
                        e.preventDefault();
                        container.querySelector('#apply-filters-btn').click();
                    }
                });
            },
            ...
            async empenhos(container, page = 1) {
                    ...
                    <div class="card">
                        <div class="filters">
                            <div class="search-bar">
                             ...
                             <button id="apply-filters-empenho-btn" class="btn">Buscar</button>
                        </div>
                    </div>
                ...
                const applyFiltersBtn = container.querySelector('#apply-filters-empenho-btn');
                const searchInput = container.querySelector('#search-empenho');

                applyFiltersBtn.addEventListener('click', () => {
                    appState.searchTerms.empenho = searchInput.value;
            ...
            async loadEmpenhosTable(page = 1) {
                ...
                try {
                    const params = new URLSearchParams({ page, size: appState.empenhos.size, numero_ne: appState.searchTerms.empenho });
                    ...
                    tableBody.innerHTML = data.results.map(e => `
                        <tr data-id="${e.id}">
                            ...
                            <td class="actions">
                                <button class="btn btn-anular" data-action="add-anulacao" data-id="${e.id}" data-numero="${e.numero_ne}">Anular/Ver Saldo</button>
                ...
                try {
                    switch (subView) {
                        case 'users':
                             container.innerHTML = `
                                ...
                                <form id="user-form" class="admin-form-grid">
                                    ...
                                    <input type="email" name="email" placeholder="E-mail do utilizador" required>
                                    ...
                                    <button type="submit" class="btn btn-primary" style="grid-column: 1 / -1;">Adicionar Utilizador</button>
                                </form>
                        ...
                        case 'secoes':
                            ...
                            const secoes = await apiService.get('/admin/secoes');
        ...
        const eventHandlers = {
            ...
            notasCredito: {
                ...
                delete(id, numero) {
                    uiComponents.showConfirmationModal('Excluir Nota de Crédito', `Tem a certeza de que deseja excluir a NC "${numero}"? Esta ação não pode ser desfeita.`, async () => {
                        try {
            ...
            empenhos: {
                ...
                delete(id, numero) {
                    uiComponents.showConfirmationModal('Excluir Empenho', `Tem a certeza de que deseja excluir o empenho "${numero}"? Esta ação não pode ser desfeita.`, async () => {
                        try {
                ...
                async openAnulacaoModal(empenhoId, numeroNe) {
                    const anulacoes = await apiService.get(`/anulacoes-empenho?empenho_id=${empenhoId}`);
                    const empenho = (await apiService.get(`/empenhos?numero_ne=${numeroNe}`)).results[0];
                    const valorTotalAnulacoes = anulacoes.reduce((acc, curr) => acc + curr.valor, 0);
                    const saldoDisponivelAnulacao = empenho.valor - valorTotalAnulacoes;

                    const anulacoesHTML = anulacoes.length > 0
                        ? anulacoes.map(a => `<tr><td>${new Date(a.data + 'T00:00:00').toLocaleDateString('pt-BR')}</td><td>${a.valor.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td><td>${a.observacao || ''}</td></tr>`).join('')
                        : '<tr><td colspan="3">Nenhuma anulação registada.</td></tr>';

                    const formHTML = `
                        <div>
                            <p><strong>Valor Original do Empenho:</strong> ${empenho.valor.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p>
                            <p><strong>Total Já Anulado:</strong> ${valorTotalAnulacoes.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p>
                            <p><strong>Saldo Disponível para Anulação:</strong> ${saldoDisponivelAnulacao.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p>
                            <hr style="margin: 1rem 0;">
                            <h4>Registar Nova Anulação</h4>
                            <form id="anulacao-form"><div class="form-grid"><div class="form-field"><label for="valor">Valor a Anular (R$)</label><input type="number" name="valor" step="0.01" min="0.01" max="${saldoDisponivelAnulacao}" required></div><div class="form-field"><label for="data">Data</label><input type="date" name="data" required value="${new Date().toISOString().substring(0, 10)}"></div><div class="form-field form-field-full"><label for="observacao">Observação</label><textarea name="observacao"></textarea></div></div><div id="form-feedback" class="modal-feedback" style="display: none;"></div><div class="form-actions"><button type="submit" class="btn btn-primary">Registar Anulação</button></div></form>
                            <hr style="margin: 1rem 0;">
                            <h4>Histórico de Anulações</h4>
                            <div class="table-container" style="max-height: 150px; overflow-y: auto;"><table><thead><tr><th>Data</th><th>Valor</th><th>Observação</th></tr></thead><tbody>${anulacoesHTML}</tbody></table></div>
                        </div>`;

                    uiComponents.openModal(`Gerir Anulações do Empenho ${numeroNe}`, formHTML, (modal, close) => {
                        modal.querySelector('#anulacao-form').addEventListener('submit', async (ev) => {
                            ...
                            try {
                                ...
                                viewRenderer.loadEmpenhosTable(appState.empenhos.page);
                            } catch(error) {
            ...
            admin: {
                 async handleUserFormSubmit(e) {
                    ...
                    const data = Object.fromEntries(new FormData(form));
                    try {
                        await apiService.post('/admin/users', data);
                        form.reset();
                ...
                async handleSecaoFormSubmit(e) {
                    ...
                    const method = id ? 'put' : 'post', endpoint = id ? `/admin/secoes/${id}` : '/admin/secoes';
                    btn.disabled = true;
                    try {
                ...
                deleteSecao(id, nome) {
                     uiComponents.showConfirmationModal('Excluir Seção', `Tem a certeza de que deseja excluir a seção "${nome}"?`, async () => {
                        try {
                            await apiService.delete(`/admin/secoes/${id}`);
                            await viewRenderer.adminSubView(document.getElementById('admin-content-container'), 'secoes');
